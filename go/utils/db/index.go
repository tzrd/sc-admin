...
// 指定表名，分页db统一返回数据
func PaginateByTable(name string, c *gin.Context, data interface{}) func(db *gorm.DB) *gorm.DB {
	return func(db *gorm.DB) *gorm.DB {
		var total int64
		page, _ := strconv.Atoi(c.Query("page"))
		if page == 0 {
		...
		offset := (page - 1) * pageSize
		db.Table(name).Model(data).Offset(-1).Count(&total)
		result := db.Table(name).Offset(offset).Limit(pageSize)
		SetTotal(c, total)
		return result
	}
}
